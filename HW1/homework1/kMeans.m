...
numP = size(P,1); % number of points
dimP = size(P,2); % dimension of points
...
while stop == false
    ...
    for idxP = 1:numP
        ...
        for idxC=1:k
            dist(idxC) = norm(double(P(idxP,:))-double(centr(idxC,:)));
        end
    ...
    for idxC = 1:k
        ...
        centr(idxC,:) = mean(P(cluster==idxC,:));
    end
